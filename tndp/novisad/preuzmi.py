...
def _upisi(ime, sadrzaj):
    izvori.RAW.mkdir(parents=True, exist_ok=True)
    put = izvori.RAW / ime
...
def preuzmi_nsmart(s):
    print("NSmart portal")
    html = s.get(f"{izvori.NSMART}/sr/prikaz-svih-linija", timeout=60).text
    if "g_in_cities" not in html:
        html = s.get(f"{izvori.NSMART}/", timeout=60).text
...
def preuzmi_gsp_mrezu(s):
    ...
    html = s.get(f"{izvori.GSP}/mreza", timeout=40).text
    ...
    for br, ln in enumerate(linije):
        for ime, url, kutija in [("stajalista", "mreza-get-stajalista-tacke", stajalista),
            ...
            r = s.get(f"{izvori.GSP}/{url}", params={"linija": ln["id"]}, timeout=30)
...
def preuzmi_zone(s):
    ...
    zone = {}
    for i in range(1, 7):
        zone[i] = s.get(f"{izvori.GSP}/cenovnik-sen/zona-mesta/{i}", timeout=30).text
        time.sleep(PAUZA)
...
def preuzmi_polaske(s):
    print("red vožnje")
    polasci = {}
    for rv in izvori.REZIMI:
        for dan in izvori.DANI:
            r = s.get(f"{izvori.GSP}/red-voznje/lista-linija",
                      params={"rv": rv, "vaziod": izvori.VAZIOD, "dan": dan}, timeout=30)
            kodovi = re.findall(r'<option value="([^"]+)">([^<]*)</option>', r.text)
            for kod, naziv in kodovi:
                r2 = s.get(f"{izvori.GSP}/red-voznje/ispis-polazaka",
                           params={"rv": rv, "vaziod": izvori.VAZIOD, "dan": dan,
                                   "linija[]": kod}, timeout=30)
                polasci[f"{rv}|{dan}|{kod}"] = {"naziv": naziv.strip(), "html": r2.text}
                time.sleep(PAUZA)
            print(f"  {izvori.REZIMI[rv]} / {izvori.DANI[dan]}: {len(kodovi)} linija")
...
def preuzmi_mesne_zajednice(s, pokusaja=5):
    ...
    for n in range(pokusaja):
        r = s.post(izvori.OVERPASS, data={"data": upit}, timeout=200)
        try:
            podaci = r.json()
        except ValueError:
...
def preuzmi_nsinfo(s):
    ...
    for ime, put in [("nsinfo_mz.html", "broj-stanovnika-po-mesnim-zajednicama"),
        ...
        _upisi(ime, s.get(f"{izvori.NSINFO}/{put}", timeout=40).text)
        time.sleep(PAUZA)
...
def _podesi_osmnx():
    ...
    ox.settings.cache_folder = str(izvori.RAW / "osmnx_cache")
...
def preuzmi_ulice():
    ...
    izvori.RAW.mkdir(parents=True, exist_ok=True)
    put = izvori.RAW / "ulice.graphml"
    if put.exists():
        print(f"  {put.name} već postoji, preskačem")
        return
    g = ox.graph_from_bbox(izvori.BBOX_ULICE, network_type="drive")
...
def preuzmi_sadrzaje():
    ...
    _podesi_osmnx()
    print("sadržaji (OSM)")
    put = izvori.RAW / "sadrzaji.geojson"
    if put.exists():
        print(f"  {put.name} već postoji, preskačem")
        return
    g = ox.features_from_bbox(izvori.BBOX_ULICE, {t: True for t in izvori.POI_TAGOVI})
    g = g.copy()
    g["geometry"] = g.geometry.representative_point()  # poligoni -> tačke
    kolone = ["geometry"] + [t for t in izvori.POI_TAGOVI if t in g.columns]
...
def main():
    ...
    preuzmi_ulice()
    preuzmi_sadrzaje()
    print(f"\ngotovo, sirovi podaci u {izvori.RAW}")

...
class TndpEnv:
    ...
    def decision(self):
        ...
        if len(self.current) < self.max_len:
            ...
            # kod linije od jednog čvora su oba kraja isti čvor, pa bi HEAD i TAIL davali istu liniju (jedna je obrnuta druga)
        ...
        # zaglavljeno: nema proširenja a kraj nije dozvoljen
        if can_halt or not mask.any():
            return HALT, mask
        return EXTEND, mask

    # Da li potez vodi u stanje iz kog se ne može izaći ispravno
    ...
    def step(self, action):
        if action == -1:
            # brojanje ide ovde a ne u decision(), jer decision() nad istim stanjem zovu i MCTS i rollout po više puta
